    ...
    public class OrdersMethods : ICustomerOrders, IOrders
    {
        public Order CreateOrderInstance(int itemID, Customer ThisCustomer, string _product)
        {
            using (var db = new Electronic_storeContext())
            {
                try
                {
                    var TestID = Convert.ToInt32(itemID);
                    var TestProduct = db.Laptops.Where(c => c.Id == TestID).Select(c => c).FirstOrDefault();

                    if (TestProduct == null)
                    {
                        throw new Exception("invalid Id Infromation");
                    }
                    else
                    {
                ...
                {
                    Debug.WriteLine("invalid infromation choose the id from the list");
                    return null;
